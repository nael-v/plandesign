...
import { formatDateTime, localizedValue, localizeSupplierCategory } from "@/lib/i18n";
...
export function ProjectWorkspace({ projectId }: Props) {
  ...
  const timelineItems = useMemo(() => {
    ...
    return activities.map((a) => ({
      ...
      timestamp: formatDateTime(locale, a.timestamp),
      color: "blue" as const,
    }));
  }, [locale, snapshotQuery.data?.activities]);

  if (snapshotQuery.isLoading) {
    return (
      <div className="space-y-4" aria-busy="true" aria-label={localizedValue(locale, { en: "Loading project workspace", he: "טוען את סביבת הפרויקט" })}>
  ...
  if (snapshotQuery.isError || !snapshotQuery.data?.project) {
    return (
      <EmptyState
        title={localizedValue(locale, { en: "Project not found", he: "הפרויקט לא נמצא" })}
        description={localizedValue(locale, { en: "This project may have been removed or you may not have access.", he: "ייתכן שהפרויקט הוסר או שאין לך הרשאה לצפות בו." })}
      />
    );
  }
    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-3xl border border-border bg-background p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-foreground">{localizedValue(locale, { en: "Team members", he: "חברי צוות" })}</h3>
            <span className="ml-auto rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
              {project.members.length}
            </span>
          </div>
          {project.members.length === 0 ? (
            <p className="text-sm text-muted-foreground">{localizedValue(locale, { en: "No team members assigned yet.", he: "עדיין לא הוקצו חברי צוות." })}</p>
          ) : (
        ...
        <div className="rounded-3xl border border-border bg-background p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-foreground">{localizedValue(locale, { en: "Suppliers", he: "ספקים" })}</h3>
            <span className="ml-auto rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
              {project.suppliers.length}
            </span>
          </div>
          {project.suppliers.length === 0 ? (
            <p className="text-sm text-muted-foreground">{localizedValue(locale, { en: "No suppliers linked yet.", he: "עדיין לא קושרו ספקים." })}</p>
          ) : (
            <ul className="space-y-2">
              {project.suppliers.map((s) => (
                <li key={s.id} className="flex items-center gap-3 rounded-xl border border-border bg-muted/30 p-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">{localizeSupplierCategory(locale, s.category)} &middot; {s.contact}</p>
                  </div>
                </li>
              ))}
            </ul>
      ...
      <section className="rounded-3xl border border-border bg-background p-6">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-foreground">{localizedValue(locale, { en: "Activity feed", he: "פיד פעילות" })}</h3>
        </div>
        {timelineItems.length === 0 ? (
          <EmptyState
            title={localizedValue(locale, { en: "No activity yet", he: "עדיין אין פעילות" })}
            description={localizedValue(locale, { en: "Actions like task updates, file uploads, and comments appear here.", he: "פעולות כמו עדכוני משימות, העלאות קבצים והערות יופיעו כאן." })}
          />
        ) : (
